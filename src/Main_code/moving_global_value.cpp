...
int main(int argc, char **argv)
{
    ...
    nextWayPoint.lat = home_value.global_pose_latitude;
	nextWayPoint.lon = home_value.global_pose_longitude;
	nextWayPoint.alt = home_value.global_pose_altitude+1;
	nextWayPoint.heading = 0;
    waypointList.push_back(nextWayPoint);
...
int counter = 1;
 ...
 while(ros::ok()){
        if( current_state.mode != "OFFBOARD" &&
            ...
            ROS_INFO("%d",counter);    
			ROS_INFO("%f",waypointList[counter].lat);

			if(set_destination_global_check(waypointList[counter].lat,waypointList[counter].lon,waypointList[counter].alt, waypointList[counter].heading)==1){
					counter++;
			}
